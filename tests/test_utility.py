...
def test_as_bytes_str() -> None:
    assert cookiebaker.utility.as_bytes("a") == b"a"
    assert cookiebaker.utility.as_bytes("é") == "é".encode("utf-8")


def test_as_bytes_bytes() -> None:
    assert cookiebaker.utility.as_bytes(b"x") == b"x"


def test_normalize_url_empty() -> None:
    u = cookiebaker.utility.normalize_url("   ")
    assert not u.isValid()


def test_normalize_url_bare_host() -> None:
    u = cookiebaker.utility.normalize_url("example.com")
    assert u.isValid()
...
def test_normalize_url_explicit_scheme() -> None:
    u = cookiebaker.utility.normalize_url("http://example.org/path")
    assert u.isValid()
...
def test_cookie_key() -> None:
    ...
    assert cookiebaker.utility.cookie_key(c) == (b"n", b"d", b"/p")
...
def test_same_site_name_default_is_omitted_in_records() -> None:
    ...
    rows = cookiebaker.utility.cookies_as_records([c])
...
def test_cookies_as_records_sorted() -> None:
    ...
    rows = cookiebaker.utility.cookies_as_records([c1, c2])
...
def test_domain_matches_via_header() -> None:
    ...
    header = cookiebaker.utility.cookie_header_for_url([c_ok, c_bad], url)
    assert header == "s=1"
...
def test_path_matches_via_header() -> None:
    ...
    header = cookiebaker.utility.cookie_header_for_url([c_api, c_root], url)
...
def test_secure_cookie_omitted_on_http() -> None:
    ...
    header = cookiebaker.utility.cookie_header_for_url([c_sec], url)
    assert header == ""
...
def test_cookie_header_with_query_in_path_matching() -> None:
    ...
    header = cookiebaker.utility.cookie_header_for_url([c], url)
    assert header == "k=v"


def test_build_payload_shape() -> None:
    c = cookie(b"n", b"v", "example.com")
    payload = cookiebaker.utility.build_payload("https://example.com/", [c])
...
def test_private_domain_matches() -> None:
    assert cookiebaker.utility._domain_matches("www.example.com", ".example.com") is True
    assert cookiebaker.utility._domain_matches("evil.com", ".example.com") is False


def test_private_path_matches() -> None:
    assert cookiebaker.utility._path_matches("/api/foo", "/api") is True
    assert cookiebaker.utility._path_matches("/apifoo", "/api") is False
